...
def command_search_handler():
    ...
    results.append({"holehe": holehe_result})

    # Fetch and filter the data
    filtered_json = fetch_and_filter_leak_data(email)
    results.append({"leak_data": filtered_json})
    ...
    if data['status'] == 1:
        if data['total_results'] != 0:
            ...
            for res in data['search_result']:
                ...
                whois += '\n'
            results.append({"whois": whois})
        else:
            results.append({"whois": 'No domain registered on this email address!'})
    ...
    try:
        ...
        results.append({"ghunt": ghunt_result})
    except Exception as e:
        results.append({"ghunt_error": str(e)})

    # DB Search
    try:
        db_results = asyncio.run(search_db(email))
        results.append({"db_search": db_results})
    except Exception as e:
        results.append({"db_search_error": str(e)})
    
    try:
        onion_results = asyncio.run(onion_search(email))
        results.append({"onion": onion_results})
    except Exception as e:
        results.append({"onion_error": str(e)})
